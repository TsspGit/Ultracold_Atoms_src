function [r]=recurrencec2f1part1(a,b,c,z,n,k)
...
% forwards, using Miller's algorithm, inside the curves shown in Figure 6.%
...
% Output: r=Computed value of 2F1(1-a-k,1-b;2-c+k;z)                      %
%-------------------------------------------------------------------------%
...
f1=gamma(1+b-c)*gamma(1+a-c)/gamma(a)/gamma(1-c)/gamma(2-c)...
    *hypergeom([1-a,1-b],2-c,z);
v(end)=0;v(end-1)=1;
...
for i3=1:k+1
    f(i3)=f1/v(1)*v(i3)/(-z/(1-z)^2)^(i3-1)*gamma(a+i3-1)*gamma(-c+i3)...
        *gamma(1-c+i3)/gamma(b-c+i3)/gamma(1+a-c+2*(i3-1));
end
