function [bp]=buchholzpoly(br,bi,zr,zi,n)
...
% Computes the n-th Buchholz polynomial pn(b,z) as defined in (3.19) in   %
% dissertation                                                            %
...
%         zr=Re(z)                                                        %
%         zi=Im(z)                                                        %
%         n =Index of Buchholz polynomial pn(b,z) defined in (3.19)       %
% Output: bp=Computed value of pn(b,z)                                    %
%-------------------------------------------------------------------------%

% Compute b,z in terms of br,bi,zr,zi
b=br+bi*1i;
z=zr+zi*1i;

% First 172 Bernoulli numbers
C=[1.000000000000000,-0.500000000000000,0.166666666666667,0,-0.033333333333333,0,...
    0.023809523809524,0,-0.033333333333333,0,0.075757575757575,0,...
    -0.253113553113554,0,1.166666666666671,0,-7.092156862745155,0,...
    54.971177944862056,0,-5.291242424242417e+002,0,6.192123188405845e+003,0,...
...
% Compute each factorial in (3.19)
H=zeros(1,1);
H(1)=1;
for i=2:fix(n/2)+1
    H(i)=H(i-1)*(n-2*i+3)*(n-2*i+4)/(2*i-2)/(2*i-3);
end

% Compute fs(b) as in (3.19)
F=zeros(1,1);
F(1)=1;
for s=2:fix(n/2)+1
    f1=4^(s-1);
    f2=f1*abs(C(2*(s-1)+1))/(s-1)*F(1);
    f3=f2;
    for r=2:s-1
        f1=f1*(2*s-2*r+1)*(2*s-2*r)/r/(r-1)/4;
        f2=f1*abs(C(2*(s-r)+1))/(s-r)*F(r);
        f3=f3+f2;
    end
    F(s)=-(b/2-1)*f3;
end

% Compute gs(z) as in (3.19)
G=zeros(1,1);
G(1)=1;
for m=2:n+1
    g1=4;
    ...
    g3=g2;
    for k=2:fix(m/2)
        g1=g1*(n-2*k+1)*(n-2*k+2)/(2*k-2)/(2*k-3)*4/k;
        g2=g1*abs(C(2*k+1))*G(m-2*k+1);
        g3=g3+g2;
    end
    G(m)=-1i*z/4*g3;
end

% Sum up to compute series in (3.19)
a1=H(1)*F(1)*G(n+1);
b1=a1;
for t=2:fix(n/2)+1
    a1=H(t)*F(t)*G(n-2*t+3);
    b1=b1+a1;
end

% Use result to find pn(b,z)
bp=(1i*z)^n/factorial(n)*b1;