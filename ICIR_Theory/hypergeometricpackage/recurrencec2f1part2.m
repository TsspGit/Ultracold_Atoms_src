function [r]=recurrencec2f1part2(a,b,c,z,n,k)
...
% forwards, using Miller's algorithm, between curves shown in Figure 6.   %
...
% Output: r=Computed value of 2F1(a+k,b;1+a+b-c+2k;1-z)                   %
%-------------------------------------------------------------------------%
...
f1=gamma(1+b-c)*gamma(1+a-c)/gamma(1-c)/gamma(1+a+b-c)...
    *hypergeom([a,b],1+a+b-c,1-z);
v(end)=0;v(end-1)=1;
...
for i3=1:k+1
    f(i3)=f1/v(1)*v(i3)*gamma(-c+i3)*gamma(1+a+b-c+2*(i3-1))...
        /gamma(b-c+i3)/gamma(1+a-c+2*(i3-1));
end
