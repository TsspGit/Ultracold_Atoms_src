function [aq]=adquad1f1boole(a,b,z,h,tol1,tol2)
...
% a,b and variable z by applying the method of adaptive quadrature with   %
% Boole's rule as detailed in Appendix G.5.                               %
...
while x<1
    ...
    % Two quadrature methods used, composite trapezoidal and Boole's rule
    quad1=h/2*(f(x)+f(x+h));
    quad2=h/90*(7*f(x)+32*f(x+h/4)+12*f(x+h/2)+32*f(x+3*h/4)+7*f(x+h));
